#include "SpoutAdapter.h"

SpoutAdapter::SpoutAdapter() : Spout() {}
SpoutAdapter::~SpoutAdapter() {}

void SpoutAdapter::AdapterSetSenderName(std::string& sendername) {
    const char* convertedSenderName = sendername.c_str();

    this->SetSenderName(convertedSenderName);
}
...
//                  unsigned int height, bool bInvert = true, GLuint HostFBO =
//                  0);
// // Send image pixels
...
// int GetSenderAdapter(const char* sendername, char* adaptername = nullptr,
//                      int maxchars = 256);
// // Get the current adapter description
// bool GetAdapterInfo(char* renderdescription, char* displaydescription,
//                     int maxchars);
...
//                     char* displayversion, int maxsize, bool& bUseDX9);
...
// bool CreateReceiver(char* Sendername, unsigned int& width, unsigned int&
// height,
//                     bool bUseActive = false);
// // Check receiver connection
// bool CheckReceiver(char* Sendername, unsigned int& width, unsigned int&
// height,
//                    bool& bConnected);
// // Receive OpenGL texture
// bool ReceiveTexture(char* Sendername, unsigned int& width, unsigned int&
// height,
//                     GLuint TextureID = 0, GLuint TextureTarget = 0,
//                     bool bInvert = false, GLuint HostFBO = 0);
// // Receive image pixels
// bool ReceiveImage(char* Sendername, unsigned int& width, unsigned int&
// height,
...
// bool DrawSharedTexture(float max_x = 1.0, float max_y = 1.0, float aspect
// = 1.0,
...
std::unique_ptr<SpoutAdapter> new_spout_adapter() {
    return std::unique_ptr<SpoutAdapter>(new SpoutAdapter());
}